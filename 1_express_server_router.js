...
app.get('/index',function(req,res){
    ...
    //向客户端输出一段HTML响应消息
    res.send('<h1>欢迎访问！这里是首页</h1>');      //html字符串
})

//GET  /user/login
app.get('/user/login',function(req,res){
    //向客户端输出一个文件中所有的内容——任意静态文件
    res.sendFile('D:\\z_project\\public\\login.html'); //任意类型的文件
})
//GET  /user/register
...
//GET /css/b
app.get('/css/b',function (req,res) {
...
app.get('/users',function(req,res){
    ...
    res.json(arr); //把服务器端数组转化为JSON字符串并输出
})
...
app.get('/products',function(req,res){
    res.send('您查询的关键字为:'+req.query.kw+'<br>您要显示的页号:'+req.query.pno);
});

//GET /do/login2?uname=tom&upwd=123
app.get('/do/login2',function(req,res){
    console.log('登录用户名：',req.query.name);
